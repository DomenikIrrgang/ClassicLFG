CLassicLFGPlayerList = {}
CLassicLFGPlayerList.__index = CLassicLFGPlayerList

setmetatable(CLassicLFGPlayerList, {
  __call = function (cls, ...)
    return cls.new(...)
  end,
})

function CLassicLFGPlayerList.new(parent, width, height)
    local self = setmetatable({}, CLassicLFGPlayerList)
...
function CLassicLFGPlayerList:Init(count)
    local Item1 = CLassicLFGPlayerListItem(nil, self.Frame, "TOPLEFT", 0)
    local Previous = CLassicLFGPlayerListItem(nil, Item1.Frame, "BOTTOMLEFT", 4)
    ...
    for i=3,count do
        local Next = CLassicLFGPlayerListItem(nil, Previous.Frame, "BOTTOMLEFT", 4)
        self.Entries[i] = Next
        Next.Frame:Hide()
        Previous = Next
    end
...
function CLassicLFGPlayerList:SetDungeonGroups(dungeonGroups)
...
function CLassicLFGPlayerList:SortGroupByFriendsAndGuild(dungeonGroups)
